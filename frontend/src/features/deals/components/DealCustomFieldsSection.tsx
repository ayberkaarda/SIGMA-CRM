...
export function DealCustomFieldsSection({ fields, values, onChange, errors }: DealCustomFieldsSectionProps) {
  ...
  return (
    <div className="flex flex-col gap-4 border-t border-border-subtle pt-4">
      <p className="text-xs font-medium uppercase tracking-wide text-fg-muted">Özel Alanlar</p>
            ...
            return (
              <div key={field.id} className="flex flex-col gap-1.5 sm:col-span-2">
                ...
                <div className="flex flex-col gap-1 rounded-md border border-border-strong bg-surface-2 p-2">
                  {options.length === 0 && <p className="px-1 py-1 text-xs text-fg-muted">Seçenek tanımlı değil.</p>}
                  {options.map((opt) => (
